...
contract AstariaRouter is Auth, Pausable, IAstariaRouter {
    ...
    function validateCommitment(IAstariaRouter.Commitment calldata commitment, address borrower)
        public
        returns (bool valid, IAstariaRouter.LienDetails memory ld)
    {
        ...
        (leaves, ld) = IStrategyValidator(strategyValidators[commitment.lienRequest.nlrType]).validateAndParse(
            commitment.lienRequest, borrower, commitment.tokenContract, commitment.tokenId
        );
     ...
     */
    function buyoutLien(uint256 position, IAstariaRouter.Commitment memory incomingTerms) external whenNotPaused {
     ...
     */
    function requestLienPosition(IAstariaRouter.Commitment calldata params, address borrower)
        external
        whenNotPaused
        onlyVaults
        returns (uint256)
    ...
    function getStrategistFee(uint256 amountIn) external view returns (uint256) {
        return amountIn.mulDivDown(STRATEGIST_ORIGINATION_FEE_NUMERATOR, STRATEGIST_ORIGINATION_FEE_BASE);
    }
    ...
    function getProtocolFee(uint256 amountIn) external view returns (uint256) {
        return amountIn.mulDivDown(PROTOCOL_FEE_NUMERATOR, PROTOCOL_FEE_BASE);
    }
