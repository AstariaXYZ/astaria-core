...
contract ClearingHouse is AmountDeriver, Clone, IERC1155, IERC721Receiver {
    ...
    function _getStorage() internal pure returns (ClearingHouseStorage storage s) {
    ...
    function setAuctionData(ILienToken.AuctionData calldata auctionData) external {
    ...
    function _execute(
        ...
        ERC20(paymentToken).safeTransfer(s.auctionStack.liquidator, liquidatorPayment);
        ...
        ASTARIA_ROUTER.LIEN_TOKEN().payDebtViaClearingHouse(
            paymentToken, collateralId, payment - liquidatorPayment, s.auctionStack.stack
        );

        if (ERC20(paymentToken).balanceOf(address(this)) > 0) {
            ERC20(paymentToken).safeTransfer(
                ASTARIA_ROUTER.COLLATERAL_TOKEN().ownerOf(collateralId), ERC20(paymentToken).balanceOf(address(this))
            );
        }
    ...
    function validateOrder(Order memory order) external {
        ...
        ERC721(order.parameters.offer[0].token).approve(
            ASTARIA_ROUTER.COLLATERAL_TOKEN().getConduit(), order.parameters.offer[0].identifierOrCriteria
        );
        ASTARIA_ROUTER.COLLATERAL_TOKEN().SEAPORT().validate(listings);
    }

    function transferUnderlying(address tokenContract, uint256 tokenId, address target) external {
        IAstariaRouter ASTARIA_ROUTER = IAstariaRouter(_getArgAddress(0));
        require(msg.sender == address(ASTARIA_ROUTER.COLLATERAL_TOKEN()));
    ...
    function settleLiquidatorNFTClaim() external {
        ...
        require(msg.sender == address(ASTARIA_ROUTER.COLLATERAL_TOKEN()));
        ClearingHouseStorage storage s = _getStorage();
        ASTARIA_ROUTER.LIEN_TOKEN().payDebtViaClearingHouse(address(0), COLLATERAL_ID(), 0, s.auctionStack.stack);
    }
}
