...
contract CollateralToken is AuthInitializable, ERC721, IERC721Receiver, ICollateralToken, ZoneInterface {
    ...
    function initialize(
        ...
        s.CONDUIT_CONTROLLER.updateChannel(address(s.CONDUIT), address(SEAPORT_), true);
    }
    ...
    function liquidatorNFTClaim(OrderParameters memory params) external {
        ...
        uint256 collateralId = params.offer[0].token.computeId(params.offer[0].identifierOrCriteria);
        address liquidator = s.LIEN_TOKEN.getAuctionLiquidator(collateralId);
        if (s.collateralIdToAuction[collateralId] == bytes32(0) || liquidator == address(0)) {
            //revert no auction
            revert InvalidCollateralState(InvalidCollateralStates.NO_AUCTION);
        }
        if (s.collateralIdToAuction[collateralId] != keccak256(abi.encode(params))) {
            //revert auction params dont match
            revert InvalidCollateralState(InvalidCollateralStates.INVALID_AUCTION_PARAMS);
        }
    ...
    function _loadCollateralSlot() internal pure returns (CollateralStorage storage s) {
    ...
    {
        CollateralStorage storage s = _loadCollateralSlot();
        return s.collateralIdToAuction[uint256(zoneHash)] == orderHash
    ...
    function isValidOrderIncludingExtraData(
        ...
        CollateralStorage storage s = _loadCollateralSlot();
        return s.collateralIdToAuction[uint256(order.parameters.zoneHash)] == orderHash
            ? ZoneInterface.isValidOrder.selector
            : bytes4(0xffffffff);
    }
    ...
    function _file(File calldata incoming) internal {
        ...
        if (what == FileType.AstariaRouter) {
            ...
            s.CONDUIT_CONTROLLER = ConduitControllerInterface(conduitController);
            (address conduit, bool exists) = s.CONDUIT_CONTROLLER.getConduit(s.CONDUIT_KEY);
            if (!exists) {
                s.CONDUIT = s.CONDUIT_CONTROLLER.createConduit(s.CONDUIT_KEY, address(this));
            } else {
                s.CONDUIT = conduit;
            }
            s.CONDUIT_CONTROLLER.updateChannel(address(s.CONDUIT), address(s.SEAPORT), true);
        } else {
        ...
        onlyOwner(collateralId)
    {
        ...
        if (s.LIEN_TOKEN.getCollateralState(collateralId) == bytes32("ACTIVE_AUCTION")) {
            revert InvalidCollateralState(InvalidCollateralStates.AUCTION_ACTIVE);
        }
        ...
        ClearingHouse(s.clearingHouse[collateralId]).transferUnderlying(addr, tokenId, address(receiver));

        //trigger the flash action on the receiver
        if (
            receiver.onFlashAction(IFlashAction.Underlying(s.clearingHouse[collateralId], addr, tokenId), data)
                != keccak256("FlashAction.onFlashAction")
        ) {
            revert FlashActionCallbackFailed();
        }

        if (securityHook != address(0) && preTransferState != ISecurityHook(securityHook).getState(addr, tokenId)) {
            revert FlashActionSecurityCheckFailed();
        }

        // validate that the NFT returned after the call

        if (IERC721(addr).ownerOf(tokenId) != address(s.clearingHouse[collateralId])) {
    ...
    function _releaseToAddress(
        ...
        ClearingHouse(s.clearingHouse[collateralId]).transferUnderlying(
            underlyingAsset.tokenContract, underlyingAsset.tokenId, releaseTo
        );
    ...
    function getUnderlying(uint256 collateralId) public view returns (address, uint256) {
        Asset memory underlying = _loadCollateralSlot().idToUnderlying[collateralId];
     ...
     */
    function tokenURI(uint256 collateralId) public view virtual override (ERC721, IERC721) returns (string memory) {
    ...
    function getClearingHouse(uint256 collateralId) external view returns (ClearingHouse) {
        return ClearingHouse(payable(_loadCollateralSlot().clearingHouse[collateralId]));
    }
    ...
    function _generateValidOrderParameters(
        ...
        offer[0] = OfferItem(ItemType.ERC721, underlying.tokenContract, underlying.tokenId, 1, 1);
        ...
        returns (OrderParameters memory orderParameters)
    {
        ...
        orderParameters =
            _generateValidOrderParameters(s, params.settlementToken, params.collateralId, prices, params.maxDuration);

        _listUnderlyingOnSeaport(s, params.collateralId, Order(orderParameters, new bytes(0)));
    }

    function _listUnderlyingOnSeaport(CollateralStorage storage s, uint256 collateralId, Order memory listingOrder)
        internal
    {
        ...
        s.collateralIdToAuction[collateralId] = keccak256(abi.encode(listingOrder.parameters));
    }
    ...
    function _settleAuction(CollateralStorage storage s, uint256 collateralId) internal {
        ...
        if (incomingAsset.tokenContract == address(0)) {
            ...
            if (s.clearingHouse[collateralId] == address(0)) {
                address clearingHouse = ClonesWithImmutableArgs.clone(
                    ...
                    abi.encodePacked(
                        address(s.ASTARIA_ROUTER), uint8(IAstariaRouter.ImplementationType.ClearingHouse), collateralId
                    )
                );
            ...
            s.idToUnderlying[collateralId] = Asset({tokenContract: msg.sender, tokenId: tokenId_});
