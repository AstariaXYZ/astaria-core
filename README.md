...
### NFTBondController
In the `NFTBondController` new loans are initiated against specific `BondVault`s. The borrower provides their `tokenId` from the `ERC721Wrapper` and loan parameters. Those parameters are validated against the merkle root of the `BondVault`. The ERC721 token is transferred to the `NFTBondController` address until repayment or liquidation occurs.
...
### Collateral Types
Collateral types will be used as a hash component to generate a unique ERC721 `tokenId`. The `tokenId` will be utilized in the `NFTBondController` to separate the concerns of collateral lock up, liquidation, and lien position management.
...
The only drawback to this design is the user will need to provide their amount and nonce as a component of the verifying hash in the `NFTBondController`. The benefits are that the appraiser can provide lending appraisals on fungible token type.
